...
import {ParticipantList} from 'client/components/participant-list/participant-list';
...
@View({
    ...
    directives: [RouterLink, MasterForm, MasterList, ParticipantForm, UploadXLSX]
})
export class EventFormPage extends MeteorComponent {
    ...
    addParticipantsToEvent(participantsData) {
        console.log('EventFormPage -> addParticipantsToEvent -> participantsData: ' + JSON.stringify(participantsData));

        this.eventModel.masters.push(participantsData)
        
        (<Control>this.eventForm.controls['participants']).updateValue(this.eventModel.masters);
    }
    // TODO: Remove this after prototyping
    get diagnostic() { return JSON.stringify(this.eventModel); }
